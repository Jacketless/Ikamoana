...
if __name__ == "__main__":
    ...
    list = ['output', 'mode', 'reps',
            ...
            'Kfile', 'dKdx_file', 'dKdy_file', 'diffusion_boost', 'diffusion_scale', 'taxis_scale']
    ...
    if plist['diffusion_scale'] is None:
        plist['diffusion_scale'] = 1
    if plist['taxis_scale'] is None:
        plist['taxis_scale'] = 1
    ...
    plist['reps'] = int(plist['reps'])
    for rep in range(plist['reps']):
        if plist['reps'] > 1:
        ...
        SIMPODYM(forcingU=plist['Ufile'], forcingV=plist['Vfile'], forcingH=plist['Hfile'],
                 ...
                 diffusion_boost=float(plist['diffusion_boost']), diffusion_scale=float(plist['diffusion_scale']),
                 taxis_scale=float(plist['taxis_scale']), dH_dxfile=plist['dHdx_file'], dH_dyfile=plist['dHdy_file'],
