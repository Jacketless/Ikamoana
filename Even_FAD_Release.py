...
def createEvenStartingDistribution(grid, lon_range=[110, 290], lat_range=[-20,20]):
    data = np.zeros(np.shape(grid.U.data[[0,1],:,:]), dtype=np.float32)
    ...
    for x in range(np.where(lons == lon_range[0])[0], np.where(lons == lon_range[1])[0]):
        for y in range(np.where(lats == lat_range[0])[0], np.where(lats == lat_range[1])[0]):
            data[0,y,x] = isOcean(grid.U.data[0,y,x])
...
def EvenFADRelease(filenames, variables, dimensions, fad_density,
                   timestep=21600, time=3, seed_timestep=7, start_time=757382400,
    ...
    grid.U.write(output_file)

    StartField = createEvenStartingDistribution(grid)
...
if __name__ == "__main__":
    ...
    variables = {'U': 'u', 'V': 'v'}
    dimensions = {'lon': 'xu_ocean', 'lat': 'xy_ocean', 'time': 'Time'}
    if not raijin_run:
